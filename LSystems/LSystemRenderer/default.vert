...
void main()
{
	mat4 combined_model = model * transforms[uint(tr_ind)];

	vert_normal = mat3(transpose(inverse(combined_model))) * v_norm;
	frag_position = vec3(combined_model * vec4(v_pos, 1.0f));
	tex_coords = t_coord;
